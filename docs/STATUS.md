...
最終更新: 2026-06-30 / 現在のバージョン: **v1.4.0**
...
- あだ名（相棒ポケモンに名前を付けられる）（v1.2.0 新規）
- 全1010体のタイプデータ＋タイプ色（タイプ色チップ・タイプ絞り込み・タイプ名でも検索）（v1.2.0 新規）
...
- カーソルをよける（既定 ON）。カーソル直下回避、ふつう/つよい の退避強度、最近操作中の busy reaction を含む
...
現時点では無効・未実装。近日対応予定のロードマップ項目：

- おさんぽ（Roam）— 画面内の自律移動、idle/sleep 休止、性格プリセット別タイミング
- せいかく（Personality）— 性格プリセットによる追従スタイル変更
- 作業見守りモード — 25/5・50/10 タイマー、休憩/作業切り替わり通知
- はじっこで休む（Edge Rest）— カーソル静止後に画面端・ウィンドウ端へ移動
