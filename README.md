...
**[→ ディスクイメージ（.dmg）をダウンロード](https://github.com/naochan3/pokefollower-desktop/releases/download/v1.0.4/PokeFollower-1.0.4-arm64.dmg)**（[.zip 版](https://github.com/naochan3/pokefollower-desktop/releases/download/v1.0.4/PokeFollower-1.0.4-arm64-mac.zip)）
...
**[→ AppImage をダウンロード](https://github.com/naochan3/pokefollower-desktop/releases/download/v1.0.4/PokeFollower-1.0.4.AppImage)**
...
```bash
chmod +x PokeFollower-1.0.4.AppImage
./PokeFollower-1.0.4.AppImage
```
...
- **通知コンパニオン（任意）** — 設定で ON にすると、許可された通知イベントを短く要約し、ポケモンの近くへドット風の吹き出しとして表示します。既定は OFF で、通知本文は保存しません。
...
- 通知コンパニオンは表示基盤とテスト通知までです。OS 全体の通知取得は macOS / Windows の権限・API 境界を確認しながら、明示的に許可された範囲だけを後続実装します。
...
生成物：`release/PokeFollower Setup <version>.exe`（例: `... 1.0.4.exe`）
