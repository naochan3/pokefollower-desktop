...
expect(/const \{ exportCodexPet \} = require\("\.\/codex-pet-export\.js"\);/.test(main), "main.js must use the Codex pet export helper");
expect(/ipcMain\.handle\("codex-pet:export-current", async \(event, packKey\) => \{[\s\S]*requireSettingsSender\(event\)[\s\S]*exportCodexPet\(\{[\s\S]*BrowserWindow,[\s\S]*root: ROOT,[\s\S]*packReader,[\s\S]*packKey: packKey \|\| settingsStore\.get\("pack"\),/.test(main), "codex-pet export IPC must require the settings window sender and export the selected pack");
