...
const boundaries = fs.readFileSync(path.join(root, "docs", "notification-permission-boundaries.md"), "utf8");
const errors = [];
...
for (const issue of [
  ...
  ["#42", "通知コンパニオンの OS 通知連携", "通知コンパニオンの権限境界"],
  ["#43", "相棒感を強める全体ロードマップ", "#36〜#41/#51 は完了済み"],
]) {
...
expect(status.includes("[通知コンパニオンの権限境界](notification-permission-boundaries.md)"), "STATUS must link notification permission boundaries");
expect(readme.includes("[通知コンパニオンの権限境界](docs/notification-permission-boundaries.md)"), "README must link notification permission boundaries");
...
expect(boundaries.includes("既定 OFF"), "permission boundaries must require default OFF");
expect(boundaries.includes("明示"), "permission boundaries must require explicit user permission");
expect(boundaries.includes("OS 通知本文はログ"), "permission boundaries must prohibit storing OS notification bodies");
expect(boundaries.includes("アプリ内イベント") && boundaries.includes("Codex notify bridge"), "permission boundaries must keep safe input scope");
expect(boundaries.includes("UNUserNotificationCenter"), "permission boundaries must document macOS boundary");
expect(boundaries.includes("User Notification Listener capability"), "permission boundaries must document Windows listener capability");
expect(boundaries.includes("Desktop Notifications Specification"), "permission boundaries must document Linux freedesktop boundary");
expect(boundaries.includes("org.freedesktop.portal.Notification"), "permission boundaries must document XDG portal boundary");
expect(
  boundaries.includes("RemoveNotification") && boundaries.includes("ClearNotifications") && boundaries.includes("使わない"),
