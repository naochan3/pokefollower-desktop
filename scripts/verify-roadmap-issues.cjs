...
expect(status.includes("現在のバージョン: **v1.3.0**"), "STATUS must state current version v1.3.0");
expect(status.includes("現在含まれているもの（v1.3.0）"), "STATUS must have included-assets section for v1.3.0");
...
expect(readme.includes("releases/download/v1.3.0/PokeFollower-1.3.0.AppImage"), "README must link the v1.3.0 Linux AppImage asset");
