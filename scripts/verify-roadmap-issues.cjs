...
expect(status.includes("現在のバージョン: **v1.0.2**"), "STATUS must state current version v1.0.2");
expect(status.includes("現在含まれているもの（v1.0.2）"), "STATUS must have included-assets section for v1.0.2");
...
expect(status.includes("全画面の自動非表示は **Windows のみ**。"), "STATUS must keep Windows-only fullscreen limitation");
